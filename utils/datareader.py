...
class DataReader():
    ...
    def __init__(self, args):
        ...
        assert len(data['features'])==len(data['adj_list'])==len(data['labels']), \
               "Graphs Number Mismatch, Possible Reason: due to unsuccessive graph indicator, \
                some gids are not existed in original indicator files, only thing is filtering graph labels. \
                Remember that unsuccessive graph indicator is okay, graph labels-graphs are corresponding by \
...
class GraphData(torch.utils.data.Dataset):
    def __init__(self, datareader: DataReader, gids: list):
        self.idx = gids
        # self.idx_new = list(range(len(gids)))
        self.rnd_state = datareader.rnd_state
        self.set_fold(datareader.data)
        # self.edge_index = self.make_edge_index(datareader.data['adj_list'])
    ...
    def make_edge_index(self, all_adj):
        all_edge_index = []
        adj_train = [all_adj[gid] for gid in self.idx]
        for adj in adj_train:
            non_zero_indices = np.triu(np.array(adj), k=1).nonzero()
            edge_index_list = [[], []]
            for i, j in zip(non_zero_indices[0], non_zero_indices[1]):
                edge_index_list[0].append(i)
                edge_index_list[1].append(j)
            all_edge_index.append(edge_index_list)
